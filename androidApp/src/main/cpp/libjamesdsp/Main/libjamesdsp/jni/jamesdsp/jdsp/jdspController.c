...
void JamesDSPSetSampleRate(JamesDSPLib *jdsp, float new_sample_rate, int forceRefresh)
{
	...
	// NAVIC FIX (self-deadlock): jdsp_unlock() is intentionally called here, BEFORE the
	// forceRefresh block below, instead of after it (as upstream originally had it).
	// CrossfeedEnable(), ArbitraryResponseEqualizerEnable() and StereoEnhancementRefresh()
	// each call jdsp_lock()/jdsp_unlock() internally (see crossfeed.c, arbEqConv.c,
	// stereoEnhancement.c). m_in_processing is a plain pthread_mutex_t initialized with a
	// NULL attr (non-recursive, non-error-checking default type on Android/Bionic) via
	// pthread_mutex_init(&jdsp->m_in_processing, NULL) above in JamesDSPCtor. Calling any of
	// those three functions while still holding the lock acquired a few lines above is a
	// same-thread self-deadlock: pthread_mutex_lock() blocks forever waiting on a mutex this
	// same thread already owns, and since the type is not error-checking it does NOT return
	// EDEADLK -- it just hangs. This reproduced 100% of the time forceRefresh=1 was passed
	// (e.g. from JamesDspWrapper.setSamplingRate(..., forceRefresh=true) on the first format
	// change), freezing the audio thread with no error, crash, or logcat output.
	// Elsewhere in this file, JamesDSPRefreshConvolutions() already calls CrossfeedEnable()
	// and ArbitraryResponseEqualizerEnable() WITHOUT wrapping them in jdsp_lock/unlock,
	// confirming these are meant to be self-locking, call-unlocked entry points -- the bug
	// here was JamesDSPSetSampleRate() alone wrapping them in an extra, redundant outer lock.
	// Unlocking before the forceRefresh block (rather than after) lets each self-locking
	// function acquire/release the mutex on its own, matching that established convention.
	jdsp_unlock(jdsp);
	if (forceRefresh)
	{
